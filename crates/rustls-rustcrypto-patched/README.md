> **OxiSQL vendored patch.** This is a copy of
> [`rustls-rustcrypto`](https://github.com/RustCrypto/rustls-rustcrypto) vendored
> into the OxiSQL workspace and applied via `[patch.crates-io]` in the root
> `Cargo.toml`. It exists to fix **RUSTSEC-2026-0104** — a CRL-parsing panic in
> `rustls-webpki` 0.102.x. The patched copy drops the vulnerable
> `rustls-webpki` dependency and routes `alg_id` through `rustls-pki-types`
> instead. It is **not** a workspace member; it is a `[patch]` source only. The
