...
pub fn op_auto_commit(
    ...
    if *auto_commit != conn.auto_commit.get() {
        if *rollback {
            // Roll back all dirty pages, reset flush/checkpoint state machines,
            // and undo any WAL frames appended during this transaction.
            pager.rollback();
            // Reset transaction state to None and re-enable autocommit,
            // mirroring the turso TxOp::Rollback arm.
...
pub fn op_savepoint(
    ...
    // Re-entry path: a RELEASE of a transaction-owner savepoint triggered an
    // async commit that has not finished yet.  Delegate back to commit_txn.
    if state.commit_state == CommitState::Committing {
        return match program.commit_txn(pager.clone(), state, mv_store)? {
            super::StepResult::Done => Ok(InsnFunctionStepResult::Done),
            super::StepResult::IO => Ok(InsnFunctionStepResult::IO),
    ...
    match op {
        SavepointOp::Begin => {
            ...
            if is_autocommit {
                // Open a write transaction eagerly so that the WAL frame
                // snapshot captured below is authoritative.
        ...
        SavepointOp::Release => {
            ...
            if is_owner {
                // This savepoint implicitly started the write transaction.
                // Commit it now — the same async path used by AutoCommit.
                conn.auto_commit.replace(true);
                return match program.commit_txn(pager.clone(), state, mv_store)? {
                    super::StepResult::Done => Ok(InsnFunctionStepResult::Done),
                    super::StepResult::IO => Ok(InsnFunctionStepResult::IO),
            ...
            // Non-owner RELEASE: changes are merged into the outer scope.
            Ok(InsnFunctionStepResult::Done)
        }

        SavepointOp::RollbackTo => {
            pager.rollback_to_savepoint(name)?;
            // The transaction remains active; only undo changes since savepoint.
            Ok(InsnFunctionStepResult::Done)
        }
    }
}
