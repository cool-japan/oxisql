...
/// Identifies which SAVEPOINT operation is being performed.
#[derive(Debug, Clone, PartialEq)]
pub enum SavepointOp {
    /// `SAVEPOINT name` — open a new savepoint.
    Begin,
    /// `RELEASE [SAVEPOINT] name` — release (and possibly commit) a savepoint.
    Release,
...
pub enum Insn {
    ...
    ///
    /// Implements all three savepoint operations through a single opcode.
    /// The `op` field selects which operation to perform; `name` is the
