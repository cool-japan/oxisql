...
use crate::storage::btree::MINIMUM_CELL_SIZE;
...
pub struct WalHeader {
    ...
    pub page_size: u32,
    /// Checkpoint sequence number. Increases with each checkpoint that fully
    /// backfills the WAL into the main database file (see
    /// [`crate::storage::wal::WalFile::reset_wal_file`]), regardless of the
    /// requested [`crate::storage::wal::CheckpointMode`].
    pub checkpoint_seq: u32,
    /// Random value used for the first salt in checksum calculations.
    /// Incremented (not re-randomized) with each checkpoint that fully
    /// backfills the WAL, matching SQLite's own convention for distinguishing
    /// WAL "generations" during crash recovery.
    pub salt_1: u32,
    /// Random value used for the second salt in checksum calculations.
    /// A different random value is generated for each checkpoint that fully
    /// backfills the WAL.
    pub salt_2: u32,
...
impl PageContent {
    ...
    /// Validate and return the b-tree page type, surfacing a corrupt/invalid
    /// page-type byte as a typed `Corrupt` error instead of panicking.
    ///
    /// Unlike the infallible [`Self::page_type`] (which `unwrap`s the
    /// discriminant conversion), this is meant to be called at b-tree
    /// interpretation entry points that read a page whose bytes may be
    /// attacker-controlled: a child / rightmost pointer on a corrupt page can
    /// reference an arbitrary, non-b-tree page whose first byte is not a valid
    /// page type. Once a page has been validated here, subsequent infallible
    /// `page_type()` calls on the same page are safe.
    pub fn validate_btree_page_type(&self) -> Result<PageType> {
        self.read_u8(0).try_into()
    }
    /// Shared, read-only view of the whole page buffer.
    ///
    /// Deliberately implemented via the *same* raw-pointer route as
    /// [`Self::as_ptr`] (i.e. `RefCell::as_ptr`, not `RefCell::borrow`) rather
    /// than going through `self.buffer.borrow()`: the latter would tie the
    /// returned slice's lifetime to a transient `Ref` guard local to this
    /// function, which cannot outlive the call — but the buffer is actually
    /// valid for as long as the page itself is (see the safety note on
    /// [`Self::as_ptr`]), and callers rely on getting a `&[u8]`/`&mut [u8]`
    /// whose lifetime tracks `&self`, not some guard.
    ///
    /// Unlike [`Self::as_ptr`], this is unconditionally sound to call any
    /// number of times, in any order, nested or not: shared references never
    /// invalidate each other under Rust's aliasing model, regardless of how
    /// many are alive at once or how they're interleaved. Prefer this over
    /// `as_ptr` for every call site that only reads the buffer -- which is
    /// most of them (`read_*`, cell/header introspection, corruption-check
    /// assertions, ...). See `as_ptr`'s doc comment for what's still required
    /// of the *write* side.
    pub fn as_slice(&self) -> &[u8] {
        let buffer_ptr: *mut Buffer = self.buffer.as_ptr();
        unsafe { (*buffer_ptr).as_slice() }
    }
    /// Exclusive view of the whole page buffer, fabricated from `&self`.
    ///
    /// # Why this is sound
    ///
    /// This crosses `RefCell`'s dynamic borrow check via `RefCell::as_ptr`
    /// (a raw pointer, never itself subject to Stacked-Borrows retagging)
    /// rather than `RefCell::borrow_mut` -- deliberately, because the intent
    /// is for the returned slice's lifetime to track the page's own lifetime
    /// (`&self`), not a transient `RefMut` guard local to this function.
    ///
    /// That means -- same as `RefCell::as_ptr` itself, or `UnsafeCell::get`
    /// -- soundness is **not** enforced by the type system here; it depends
    /// entirely on the actual, dynamic access pattern upholding Rust's
    /// aliasing rules: at most one live `&mut [u8]` into this page's buffer
    /// at a time, and never *simultaneously live* with any `&[u8]`
    /// ([`Self::as_slice`]) into it either.
    ///
    /// Concretely, every call site that reaches this method (directly, or
    /// transitively through `write_u8`/`write_u16`/`write_u32`/etc.) MUST:
    ///   - gather any metadata it needs (cell count, page type, header size,
    ///     cell offsets, ...) via [`Self::as_slice`] or the `read_*`/
    ///     introspection methods *first*,
    ///   - call `as_ptr` *last*, and
    ///   - perform its write(s) in one uninterrupted stretch with no other
    ///     `self`/`page` call (read or write) in between, then drop the
    ///     slice.
    ///
    /// Violating this was exactly the shape of the UB Miri caught here
    /// historically (e.g. a slice held across a nested call that also
    /// touched the same page): a fresh call to `as_ptr` -- or even to
    /// `as_slice` -- while an *older* `as_ptr` result is still going to be
    /// used again invalidates that older result, full stop, regardless of
    /// whether the two accesses would have touched disjoint byte ranges.
    /// There is no `UnsafeCell`-based escape hatch that relaxes this for the
    /// exclusive side (only genuinely non-overlapping-in-time access does)
    /// -- see the investigation notes in the Bug 2 fix commit/report for the
    /// empirical Miri probes that ruled it out.
    #[allow(clippy::mut_from_ref)]
    pub fn as_ptr(&self) -> &mut [u8] {
        let buffer_ptr: *mut Buffer = self.buffer.as_ptr();
        unsafe { (*buffer_ptr).as_mut_slice() }
    }
    ...
    /// Like [`Self::write_u32`] but `pos` is an absolute offset into the page
    /// buffer rather than one relative to `self.offset`. Cell content (and the
    /// cell pointer array) is always addressed in absolute, page-relative terms
    /// per the SQLite file format — even on page 1, where the b-tree header
    /// itself starts at byte 100. Used when rewriting an existing pointer
    /// (child page pointer / overflow-chain pointer) found via
    /// [`Self::cell_get_raw_region`].
    ...
    pub fn cell_get(
        ...
        let buf = self.as_slice();
        // Validate the page-type byte first: on a corrupt page it may be an
        // invalid discriminant, which would otherwise panic in the infallible
        // `page_type()` reached via `header_size()` below.
        let page_type = self.validate_btree_page_type()?;
        let ncells = self.cell_count();
        let cell_pointer_array_start = self.header_size();
        // `idx` / `ncells` may be attacker-controlled on a corrupt page; a bare
        // `assert!` (compiled in for this crate even in release) would panic, so
        // return a typed Corrupt error instead.
        if idx >= ncells {
        ...
        // The cell pointer array slot may lie past the end of the buffer if the
        // (untrusted) cell count is corrupt-large; bounds-check before reading.
        if self.offset + cell_pointer + 2 > buf.len() {
            crate::bail_corrupt_error!("Cell pointer array slot out of range");
        }
    ...
    pub fn cell_table_interior_read_rowid(&self, idx: usize) -> Result<i64> {
        ...
        // The cell pointer is a 16-bit value read straight from the page and may
        // be arbitrary on a corrupt page; validate it before indexing.
        if cell_pointer + LEFT_CHILD_PAGE_SIZE_BYTES > buf.len() {
            crate::bail_corrupt_error!("Cell pointer out of range");
        }
    ...
    pub fn cell_table_interior_read_left_child_page(&self, idx: usize) -> Result<u32> {
        ...
        let cell_pointer = cell_pointer_array_start + (idx * 2);
        let cell_pointer = self.read_u16(cell_pointer) as usize;
        // Validate the 16-bit cell pointer read from the page before indexing
        // into the left-child page pointer; a corrupt page may store garbage.
    ...
    pub fn cell_table_leaf_read_rowid(&self, idx: usize) -> Result<i64> {
        ...
        let cell_pointer = cell_pointer_array_start + (idx * 2);
        let cell_pointer = self.read_u16(cell_pointer) as usize;
        // Validate the 16-bit cell pointer read from the page before slicing;
        // a corrupt page may store an offset past the end of the buffer.
        if cell_pointer > buf.len() {
            crate::bail_corrupt_error!("Cell pointer out of range");
        }
    ...
    pub fn cell_get_raw_region(
        ...
        // Validate the page-type byte first: on a corrupt page it may be an
        // invalid discriminant, which would otherwise panic in the infallible
        // `page_type()` reached via `cell_pointer_array_offset_and_size()`.
        let page_type = self.validate_btree_page_type()?;
        let ncells = self.cell_count();
        let (cell_pointer_array_start, _) = self.cell_pointer_array_offset_and_size();
        // `idx` / `ncells` may be attacker-controlled on a corrupt page; a bare
        // `assert!` would panic, so return a typed Corrupt error instead.
        if idx >= ncells {
            crate::bail_corrupt_error!(
                "cell_get: idx out of bounds: idx={}, ncells={}",
                idx,
        ...
        let cell_pointer = cell_pointer_array_start + (idx * 2);
        // The cell pointer array slot itself may be out of range if the (untrusted)
        // cell count in the page header is corrupt; bounds-check before reading it.
        if cell_pointer + 2 > buf.len() {
            crate::bail_corrupt_error!("Cell pointer array slot out of range");
        }
        let cell_pointer = self.read_u16_no_offset(cell_pointer) as usize;
        // The 16-bit cell pointer read from the page is attacker-controlled on a
        // corrupt page. Validate it lies within the buffer before slicing into it
        // (the interior branches index `cell_pointer + 4`), so a malicious offset
        // yields a Corrupt error instead of an out-of-bounds panic.
        if cell_pointer + 4 > buf.len() {
            crate::bail_corrupt_error!("Cell pointer out of range");
        }
        ...
        let len = match page_type {
            PageType::IndexInterior => {
                let (len_payload, n_payload) = read_varint(&buf[cell_pointer + 4..])?;
            ...
            PageType::TableInterior => {
                let (_, n_rowid) = read_varint(&buf[cell_pointer + 4..])?;
                4 + n_rowid
            }
            PageType::IndexLeaf => {
                let (len_payload, n_payload) = read_varint(&buf[cell_pointer..])?;
                let (overflows, to_read) = payload_overflows(
                    len_payload as usize,
                    payload_overflow_threshold_max,
                    payload_overflow_threshold_min,
            ...
            PageType::TableLeaf => {
                let (len_payload, n_payload) = read_varint(&buf[cell_pointer..])?;
                let (_, n_rowid) = read_varint(&buf[cell_pointer + n_payload..])?;
                let (overflows, to_read) = payload_overflows(
                    len_payload as usize,
                    payload_overflow_threshold_max,
                    payload_overflow_threshold_min,
        ...
        Ok((start, len.max(MINIMUM_CELL_SIZE)))
    }
...
pub fn read_btree_cell(
    ...
    match page_type {
        PageType::IndexInterior => {
            let mut pos = pos;
            // `pos` is an attacker-controlled cell pointer on a corrupt page.
            // Validate it before indexing the 4-byte left-child pointer so a
            // bogus offset returns Corrupt rather than panicking (and so the
            // `page.len() - pos` below cannot underflow).
            if pos + 4 > page.len() {
                crate::bail_corrupt_error!("Cell pointer out of range");
            }
            let left_child_page =
            ...
            // On a truncated/corrupt page `to_read` (in the overflow branch) is
            // derived from the declared payload size and is independent of how
            // many bytes actually remain, so a valid-looking cell pointer near
            // the buffer end can make `pos + to_read` run past `page.len()`.
            // Reject that instead of slicing out of bounds.
            if pos + to_read > page.len() {
                crate::bail_corrupt_error!("cell payload extends beyond page");
            }
        ...
        PageType::TableInterior => {
            let mut pos = pos;
            // Validate the attacker-controlled cell pointer before indexing.
            if pos + 4 > page.len() {
                crate::bail_corrupt_error!("Cell pointer out of range");
            }
            let left_child_page =
        ...
        PageType::IndexLeaf => {
            let mut pos = pos;
            // Validate the attacker-controlled cell pointer so slicing and the
            // `page.len() - pos` computation below cannot panic/underflow.
            if pos > page.len() {
                crate::bail_corrupt_error!("Cell pointer out of range");
            }
            let (payload_size, nr) = read_varint(&page[pos..])?;
            ...
            let (overflows, to_read) =
                payload_overflows(payload_size as usize, max_local, min_local, usable_size);
            let to_read = if overflows { to_read } else { page.len() - pos };
            // See the IndexInterior arm: guard against an overflowing payload
            // whose on-page length points past a truncated page buffer.
            if pos + to_read > page.len() {
                crate::bail_corrupt_error!("cell payload extends beyond page");
            }
            let (payload, first_overflow_page) =
...
fn read_payload(
    ...
    if payload_size <= cell_len {
        ...
        // The overflowing case reads the trailing 4 bytes as the first overflow
        // page number. A corrupt/truncated cell can leave fewer than 4 bytes
        // here, in which case `cell_len - 4` would underflow (usize) and produce
        // a huge out-of-bounds index. Guard with checked arithmetic instead.
        let Some(overflow_ptr_start) = cell_len.checked_sub(4) else {
            crate::bail_corrupt_error!("Cell payload too small for overflow page pointer");
        };
        let first_overflow_page = u32::from_be_bytes([
            unread[overflow_ptr_start],
            unread[overflow_ptr_start + 1],
            unread[overflow_ptr_start + 2],
            unread[overflow_ptr_start + 3],
        ]);
        Ok((&unread[..overflow_ptr_start], Some(first_overflow_page)))
    }
}
#[inline(always)]
...
pub fn read_record(payload: &[u8], reuse_immutable: &mut ImmutableRecord) -> Result<()> {
    ...
    // `header_size` is attacker-controlled; a value smaller than the varint that
    // encoded it would underflow below. `assert!` is compiled in for this crate
    // even in release, so replace it with a graceful Corrupt error.
    if (header_size as usize) < nr {
        crate::bail_corrupt_error!("Invalid record header size");
    }
    ...
    while header_size > 0 {
        ...
        pos += nr;
        // Guard against an attacker-controlled header that under-counts the
        // bytes actually consumed, which would underflow `header_size`.
        if header_size < nr {
            crate::bail_corrupt_error!("Invalid record header size");
        }
        header_size -= nr;
...
pub fn read_varint(buf: &[u8]) -> Result<(u64, usize)> {
    ...
    // A 9-byte varint uses all 8 bits of the ninth byte. On a truncated/corrupt
    // buffer the ninth byte may be missing, so bounds-check before indexing
    // instead of panicking with an out-of-bounds access.
    match buf.get(8) {
        Some(c) => {
            v = (v << 8) + *c as u64;
            Ok((v, 9))
