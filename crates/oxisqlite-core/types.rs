...
impl ExternalAggState {
    pub fn cache_final_value(&mut self, value: Value) -> &Value {
        ...
        self.finalized_value
            .as_ref()
            .expect("invariant: just set above")
    }
}
...
impl Ord for Value {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.partial_cmp(other)
            .expect("invariant: Value implements total order") // UPSTREAM (Limbo): unwrap — needs proper error propagation
    }
}
        ...
        for value in &self.values {
            ...
            if value
                != self
                    .values
                    .last()
                    .expect("invariant: iterating over non-empty values")
            {
        ...
        match self {
            ...
            Self::Text(text) => ExtValue::from_text(
                std::str::from_utf8(text.value.to_slice())
                    .expect("invariant: text values are valid UTF-8") // UPSTREAM (Limbo): unwrap — needs proper error propagation
                    .to_string(),
            ),
...
impl Ord for RefValue {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.partial_cmp(other)
            .expect("invariant: RefValue implements total order") // UPSTREAM (Limbo): unwrap — needs proper error propagation
    }
}
...
pub fn compare_immutable(
    ...
    for (i, (l, r)) in l.iter().zip(r).enumerate() {
        ...
        let cmp = match (l, r) {
            ...
            _ => l
                .partial_cmp(r)
                .expect("invariant: index key comparison is total"), // UPSTREAM (Limbo): unwrap — needs proper error propagation
        };
        if !cmp.is_eq() {
