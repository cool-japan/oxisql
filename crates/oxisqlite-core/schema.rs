...
fn ref_act_to_str(act: limbo_sqlite3_parser::ast::RefAct) -> &'static str {
...
fn build_fk_def(
    ...
    for arg in &clause.args {
        match arg {
            ...
            RefArg::OnInsert(_) => {} // not emitted by SQLite pragma
        }
    }
...
fn create_table(
    ...
    // Collect FK raw data: (from_cols, clause).  We keep insertion order so
    // that `id` values match the order FKs appear in the DDL.
                ...
                for c_def in &col_def.constraints {
                    match &c_def.constraint {
                        ...
                        limbo_sqlite3_parser::ast::ColumnConstraint::ForeignKey {
                            ...
                            // Column-level inline FK: `col_name TYPE REFERENCES parent(col)`.
    ...
    // Build the ForeignKeyDef list from collected raw FK data.
